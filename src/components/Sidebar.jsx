...
export default function Sidebar() {
  ...
  return (
    <>
      ...
      <aside className={sidebarClass} onMouseEnter={() => setShowCloseIcon(true)} onMouseLeave={() => setShowCloseIcon(false)}>
        {isOpen ? (
          <>
              ...
              <div className="mt-5 px-4 flex flex-col">
                ...
                <div className="mt-2 flex flex-col gap-1 max-h-[240px] overflow-y-auto">
                  {projects.map((p) => {
                    ...
                    return (
                      <button key={p.id} onClick={() => router.push(`/chat/${p.id}`)} className={`flex cursor-pointer items-center gap-2 rounded-[5px] px-2 py-2 text-[13px] font-normal transition-all ${active ? "bg-[linear-gradient(90deg,#A64D79_0%,rgba(64,30,47,0)_100%)] text-white" : "text-white/70 hover:bg-[linear-gradient(90deg,#A64D79_0%,rgba(64,30,47,0)_100%)] hover:text-white"}`}>
